...
# TODO - could we hide some of this in an abstraction (i.e. a module)? Because, particularly for a first module, this is a lot of stuff to look at.
...
resource "aws_ecs_task_definition" "app" {
  ...
  # TODO - extract this into a template, to make this less scary?
...
resource "aws_ecs_service" "whale-service" {
  ...
  network_configuration {
    security_groups  = [aws_security_group.ecs_tasks.id] # TODO - Exercise DB - add the db access security group here. `aws_security_group.database_access.id`
...
# TODO: I could split the SGs out into an other exercise, as I did for the previous SGs - PRO of that is they're not making too many things at once, CON is it's another step that will bloat the module.
...
# TODO - other split points for this exercise: We could get them to not add the load balancer section to the ecs_service, to help them understand that when they create a target group, they're not populating it. We could also split out the public -> private section.
